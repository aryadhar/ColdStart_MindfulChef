...
def ChooseRandomEqualProb(recipesData, numChoices):
    ...
    arrayRecipeID=np.array(random.choices(fullListRecipeId, weights=np.array(recipesData['weight']), k=numChoices))
...
def ChooseRandomFoodGroupWeightedProb(recipesData, numChoices, foodGroupChoice):
    ...
    # Reading Recipe ratings (API).csv
    ratingsDF=pd.read_csv('Recipe ratings (API).csv')
    ratingsDF.columns=ratingsDF.columns.str.replace(' ', '_')
...
def ChooseRandomFoodGroupEqualProb(recipesData, numChoices, foodGroupChoice):
    

    
    allFoodGroupRecipeID=np.array([])
    allFoodGroupWeight=np.array([])
    ...
    if(numChoices>=len(foodGroupChoice)):
        ...
        for elem in foodGroupChoice:
            ...
            foodGroupWeight=np.array([recipesData.loc[recipesData['id']==i].weight.values[0] for i in foodGroupRecipeID])
         
            arrayRecipeID[counter]=np.array(random.choices(foodGroupRecipeID, foodGroupWeight, k=1))


            allFoodGroupRecipeID=np.append(allFoodGroupRecipeID,(foodGroupRecipeID), axis=0)
            allFoodGroupWeight=np.append(allFoodGroupWeight, foodGroupWeight, axis=0)

            counter+=1
            
        arrayRecipeID=np.insert(arrayRecipeID, counter, (np.array(random.choices(allFoodGroupRecipeID, allFoodGroupWeight, k=numChoices-counter))))
